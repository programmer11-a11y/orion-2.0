...
(function ($) {
  journea_travel_agency = {
    ...
    general_open: function () {
      ...
      document.addEventListener("DOMContentLoaded", () => {
        ...
        // Helper function to check if we're on careers.html or index-4.html
        const isCareersPage = () => {
          const currentPath = window.location.pathname;
          const currentPage = currentPath.split("/").pop() || "";
          return (
            currentPage === "careers.html" || currentPage === "index-4.html"
          );
        };
  ...
  <div class="md:px-0 2xl:ml-10 xl:ml-8 sm:ml-6 ml-3">
    ...
    <form id="contactForm" method="post" novalidate>
      <div class="grid grid-cols-1 sm:grid-cols-2 2xl:gap-[30px] 1xl:gap-6 xl:gap-[20px] gap-[15px]">
        ...
        <div class="relative">
          ...
          <div class="text-red-500 mt-2 opacity-0 transition-all ease-in-out error-message hidden items-center gap-2">
        ...
        <div>
          ...
          <input type="text" class="w-full border border-black xl:px-3 px-2 2xl:py-[14px] py-2 focus:outline-none" />
        </div>
      ...
      <div class="flex items-center gap-5 2xl:mt-[26px] xl:mt-5 mt-4">
        ...
        `; // keep your existing long template here (unchanged)
        ...
        (function slimReply() {
          ...
          function accClose(el, { remove = false, marginZero = false } = {}) {
            if (!el) return;
            if (marginZero) {
              el.classList.remove("mt-5", "sm:mt-6");
          ...
          // close all others (except optional 'exceptEl')
          function closeAllReplies(exceptEl) {
            const open = commentsList.querySelectorAll(".reply-sibling");
            open.forEach((el) => {
              if (el !== exceptEl)
                accClose(el, { remove: true, marginZero: true });
            });
          }
          ...
          function slideUp(el, remove = false) {
            el.style.height = el.scrollHeight + "px";
            el.style.overflow = "hidden";
          ...
          commentsList.addEventListener("click", function (e) {
            const replyBtn = e.target.closest("a");
            if (!replyBtn) return;

            const txt = (replyBtn.textContent || "").toLowerCase();
            if (!txt.includes("reply")) return;

            e.preventDefault();

            // FIX: your HTML has .comments-content, not .comments-row
            const commentRow =
              replyBtn.closest(".comments-content").parentElement;

            // Find existing reply boxes
            const openBoxes = commentsList.querySelectorAll(".reply-sibling");

            // The one directly below clicked comment
            let existing = commentRow.nextElementSibling;
            const isBox =
              existing && existing.classList.contains("reply-sibling");

            // Close all others
            openBoxes.forEach((box) => {
              if (box !== existing) slideUp(box, true);
            });

            // Toggle existing
            if (isBox) {
              slideUp(existing, true);
              return;
            }
            ...
            box.className = "reply-sibling mt-5 hidden";

            // Add left border like old JS
            box.style.borderLeft = "1px solid #00000026";

            // Detect nested comment and indent more
            const isSub = commentRow.querySelector(":scope > .border-l");
            box.style.marginLeft = isSub ? "32px" : "16px";

            box.innerHTML = REPLY_FORM_HTML;

            commentRow.insertAdjacentElement("afterend", box);
            ...
            form.addEventListener("submit", function (ev) {
              ...
              inputs.forEach((inp) => {
                ...
                // use main contact validation system
                const result = window.contactValidateField(
                  name,
                  inp.value,
                  inp,
                );
                ...
                if (!result.isValid) {
                  window.contactShowError(inp, result.message);
                  hasError = true;
                } else {
                  window.contactClearError(inp);
                }
              });

              if (hasError) return; // stop submit

              // SUCCESS — close smoothly
              slideUp(box, true);

              // Optional: Add success behavior here
            });
          });
        })();
  ...
  `;
    ...
    Preloader_js: function () {
      ...
      function lockScroll() {
        ...
        /* KEEP body scrollable for popup */
...
// ---- Hover image for FAQ / Services (patched) ----
document.addEventListener("DOMContentLoaded", function () {
  const hoverImage = document.querySelector(".hover-image");

  if (!hoverImage) return;

  const faqs = document.querySelectorAll(".faqs-list, .service-item, .accordion-box");

  // helper to set opacity/visibility safely
  function showHoverImage(imgUrl) {
    if (!imgUrl) return;
    hoverImage.src = imgUrl;
    hoverImage.style.transition = "opacity 250ms ease, transform 300ms ease";
  ...
  faqs.forEach((faq) => {
    const title = faq.querySelector(".faqs-title, .service-title, .accordion-title");
    const content = faq.querySelector(".faqs-content p, .service-content, .accordion-content");

    // ensure data-img attribute exists on the faq element (data-img="path/to/img.jpg")
    const imgUrl = faq.getAttribute("data-img") || faq.dataset.img || null;

    // if there is no img URL, nothing to do
    if (!imgUrl) return;

    // mouseenter: only if this item is currently open (has .active or aria-expanded)
    function handleEnter(e) {
      // consider it open if it already has .active, or aria-expanded="true", or .open class
      const isOpen =
        faq.classList.contains("active") ||
        faq.classList.contains("open") ||
        faq.getAttribute("aria-expanded") === "true";

      if (!isOpen) return;
      showHoverImage(imgUrl);
    }
    ...
    function handleMove(e) {
      // place image relative to a containing section (fallback to document.body)
      const container = faq.closest(".hover-image-section") || document.body;
      const rect = container.getBoundingClientRect();

      // Fix: no extra space before 'px' – correct concatenation
      const left = e.clientX - rect.left + 10;
      const top = e.clientY - rect.top + 10;

      // keep image inside container bounds (clamp)
      const imgRect = hoverImage.getBoundingClientRect();
      const maxLeft = Math.max(0, rect.width - (imgRect.width || 220));
      const maxTop = Math.max(0, rect.height - (imgRect.height || 180));

      hoverImage.style.left = Math.min(maxLeft, Math.max(0, left)) + "px";
      hoverImage.style.top = Math.min(maxTop, Math.max(0, top)) + "px";

      // rotate a little for the angled look (optional)
      hoverImage.style.transform = "rotate(-8deg)";
    }

    // attach if we have either title or content
    [title, content].forEach((hoverArea) => {
      ...
      hoverArea.addEventListener("mousemove", handleMove);
    });
  });

  // ensure only visible when needed on load/unload of active item
  const observer = new MutationObserver(() => {
    // if no active faq, hide image
    const active = document.querySelector(".faqs-list.active, .service-item.active, .accordion-box.active, .accordion-box.open");
    if (!active) hideHoverImage();
  });
...
// intel-tel-input as global add  ==========================================
/* ============================================================
   GLOBAL PHONE INPUT INITIALIZER (Works on ALL Pages)
============================================================ */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (function initUnifiedFileManager() {
    function init() {
      ...
      function showDropSuccessMessage() {
        if (!dropDefault || !dropSuccess) return;

        // Hide default
        dropDefault.classList.add("hide");

        // Show success
        dropSuccess.classList.add("show");

        // Reset timer if needed
        if (dropTimer) clearTimeout(dropTimer);

        dropTimer = setTimeout(() => {
          dropSuccess.classList.remove("show");
          dropDefault.classList.remove("hide");
        }, 4000);
      }
      ...
      function updateUI() {
        ...
        fileList.forEach((file, index) => {
          ...
          row.className =
            "w-full flex justify-between items-center bg-primary-900/12 border-b-2 border-primary-900 sm:px-5 px-[10px] sm:py-[12px] py-[5px] mt-3";
...
// custom dropdown for all form page
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".custom-dropdown").forEach((d) => {
    ...
    d.querySelectorAll(".dropdown-option").forEach((opt) => {
      opt.addEventListener("click", (e) => {
        ...
        // ✔ FIX: hide error completely
        if (errBox) {
          errBox.classList.add("hidden", "opacity-0");
          errBox.classList.remove("opacity-100", "flex");
          errText.textContent = "";
        }
      ...
      dropdowns.forEach((d) => {
        ...
        if (sel && sel.hasAttribute("required")) {
          ...
          if (!val || val.trim() === "") {
            ...
            // ✅ Show error message
            if (errBox) {
              const fieldName = d.dataset.dropdown;
              ...
              } else if (fieldName === "construction-type") {
                message = "Please select construction type";
              }

              errText.textContent = message;

              errBox.classList.remove("hidden", "opacity-0", "invisible");
              errBox.classList.add("opacity-100", "flex");
            }
          }
        }
      });
...
document.addEventListener("DOMContentLoaded", () => {
  const form = document.getElementById("contactForm"); // ADD THIS
  if (!form) return; // avoid errors on pages without form
  ...
  function showError(input, message) {
    ...
    if (errorBox) {
      ...
      errorBox.classList.add("opacity-100", "flex");
  ...
  function clearError(input) {
    ...
    if (errorBox) {
      ...
      errorBox.classList.remove("opacity-100", "flex");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (applyButtons.length && enquiryForm) {
    applyButtons.forEach(btn => {
      btn.addEventListener("click", function (e) {
        e.preventDefault();

        // small delay because your theme scripts run heavy on click
        setTimeout(() => {
          const headerHeight = document.querySelector("header").offsetHeight || 100;

          const elementPosition = enquiryForm.getBoundingClientRect().top;
